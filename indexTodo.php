...
<div class="main-section">
    <div class="add-section">
        ...
        <form action="todolist/addTodo.php" method="POST" autocomplete="off">
            <?php if (isset($_GET['mess']) && $_GET['mess'] == 'error') { ?>
                <input type="text" name="titulo" style="border-color: #ff6666" placeholder="Campo Obligatorio" />
                <button type="submit">Add &nbsp; <span>&#43;</span></button>
            <?php } else { ?>
                <input type="text" name="titulo" placeholder="Anota un pedido :)" />
                <button type="submit">Add &nbsp; <span>&#43;</span></button>
            <?php } ?>
        </form>
    </div>
    ...
    <div class="show-todo-section">
        ...
        <?php while ($todo = $todos->fetch(PDO::FETCH_ASSOC)) { ?>
            <div class="todo-item">
                <span idTodo="<?php echo $todo['idTodo']; ?>" class="remove-to-do">x</span>
                <?php if ($todo['checked']) { ?>
